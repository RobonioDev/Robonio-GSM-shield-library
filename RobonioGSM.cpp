...
void RobonioGSM::phoneSetFeature(){
  SIM.print (F("AT+CFUN=1\r\n"));
}


void RobonioGSM::signalQuality(){
/*Response
...
void RobonioGSM::CarrierOpenProfile(){
...
void RobonioGSM::CarrierCloseProfile(){
  SIM.print (F("AT+SAPBR=0,1\r\n "));
  delay(1500);
}


bool RobonioGSM::callAnswer(){
   SIM.print (F("ATA\r\n"));
   _tampon=_serialOku();
...
void  RobonioGSM::numberCall(char* number){
  SIM.print (F("ATD"));
  SIM.print (number);
  SIM.print (F(";\r\n"));
}
...
uint8_t RobonioGSM::GetSearchStatus(){
/*
  dönüş değerleri:
...
bool RobonioGSM::smsSend(char* number,char* text){
...
String RobonioGSM::NumberSmsRead(uint8_t index){
  _tampon=smsRead(index);
...
String RobonioGSM::smsRead(uint8_t index){
  SIM.print (F("AT+CMGF=1\r")); 
...
bool RobonioGSM::RTCupdate(int utc){
